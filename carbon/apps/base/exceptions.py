...
def exception_handler(exc, context=None, misc=None, **kwargs):
    ...
    if settings.DEBUG or settings.RAISE_ALL_EXCEPTIONS:
        raise Exception(
            f"the exception: "
            f"`{exc}` occurred with the (optional) context: `{context}` and these "
            f"(also optional) kwargs: `{kwargs}`."
        )
